...
while 1
    ...
    G.eps0=G.eps0+Heps;
    if G.eps0==Heps
        [e_log, func_log, endings, G, thrs]=alpha_flow2(G, k, alstart, alfinish, thrs, 0, fileID);
    else
        [e_log, func_log, endings, G, thrs]=alpha_flow2(G, k, alstart, alfinish, thrs, 1, fileID);
    end
...
[eps_fin, e_fin, ix]=getAnswerFromBack(eps_back, es_ans_back, func_fin_back);
plot(eps_back(ix), func_fin_back(ix), 'go', 'linewidth', 4);
...
simpleDrawB1(G.B1, G.w, eps_fin, e_fin, 0);
